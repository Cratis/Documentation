...
- ⚠️ **HIGH PRIORITY — Arc reference API-correctness cleanup (audit round 3 found; NOT yet fixed — verify each vs source before fixing).** Setup/reference pages teaching non-existent APIs:
  - `backend/core/getting-started.md` — **fabricated `ICommand` / `ICommandHandler<T>` / `IQuery<T>` / `IQueryHandler<T,R>` interfaces** (no match in `Arc/Source`; real = `[Command]`+`Handle()` / model-bound query methods), `app.UseCratisArc("url")` (takes **no** args; the HTTP prefix comes from `ArcOptions.Hosting.ApplicationUrl`), and `CommandResult.Success` used as a property (it's a method `Success(CorrelationId)`). Multiple examples — needs a rewrite.
  - `backend/asp-net-core/configuration.md` — `builder.UseCratisArc(...)` on `IHostBuilder` (lines 73/87/100/130/300) → `AddCratisArc(...)`; the string form is `AddCratisArc(configSectionPath: "…")` (4th **named** param, not positional — `Arc/Source/.../Arc/HostBuilderExtensions.cs:30`). The `app.UseCratisArc()` calls are correct.
  - `backend/core/authorization.md` — its deeper sections (custom auth, testing) still use the fabricated generic `ICommandHandler<T>`/`IQueryHandler` + `context.User`/`CommandResult.Forbidden`; the *primary* examples were already modernized this session.
  - *(Already fixed this session: `reducers/getting-started.md` retrieval → `_eventStore.ReadModels.GetInstanceById<T>`; the `mongodb/getting-started.md` + `arc-without-event-sourcing` bootstrap → `AddCratisArc`.)*

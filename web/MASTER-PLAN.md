...
- **Arc reference API-correctness cleanup (audit round 3) — mostly DONE; one blocked item remains.**
  ...
  - ⚠️ **`backend/core/authorization.md` — deep sections still BLOCKED.** Primary examples + the framing note are fixed; the deeper sections (Handler-Level / Working with Claims / Custom / Policy / Testing) still show the fabricated generic `ICommandHandler<T>`/`IQueryHandler` + `context.User` + `CommandResult.Forbidden(...)`. **Blocker:** the model-bound *current-user* accessor is unverified — `CommandContext` has **no** `.User`, and `IUserAccessor` is **not** an Arc type. Find the real way to read the signed-in user inside `Handle()` (or read claims off the identity payload) before rewriting these examples.

...
### In flight at last checkpoint (2026-05-31, awesome-docs pass — VERIFY THESE)
- **Content commits LANDED** on `docs-overhaul` (product repos, not pushed): Chronicle `fffe77624` (projections), Arc `d249c25c` (commands+queries) + `27e936bb` (frontend tour), Fundamentals `c59c939` (C# concept walkthrough).
- **Cross-product Glossary** authored at `web/src/content/docs/glossary.md` (one-term-one-definition, links into existing `/chronicle/concepts/*`, `/arc/backend/*`, `/fundamentals/csharp/concepts/`) + wired into the Overview topic in `astro.config.mjs`. **NOT yet build-verified** — the `check-links` gate must confirm every glossary link resolves; fix any broken ones (candidates to double-check: `/chronicle/reducers/`, `/chronicle/reactors/`, `/arc/backend/proxy-generation/`, `/fundamentals/csharp/concepts/`).
- **Prose linter strengthened** (`web/scripts/lint-docs.mjs`): added error-level authoring-marker checks (TODO/FIXME/TBD/lorem) and **warning-level** Google/MS style nudges (weasel words: simply/just/obviously/easily…; heading end-punctuation). Warnings DON'T fail the gate. Zero new deps. **Run `npm run check` to confirm errors==0** (some existing pages may now emit warnings — that's expected/non-blocking).
- These two (glossary + linter) were authored during a Bash/Read output-channel outage and are UNVERIFIED. First action next session: `cd web && npm run check`, confirm 0 errors / 0 broken, fix glossary links if any broke, then commit glossary (Documentation repo) + linter.

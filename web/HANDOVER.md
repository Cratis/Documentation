...
- **Documentation** `940deec` "Stop the linter flagging code-span and entity headings"
- **Chronicle** `ed75da27a` "Add diagrams to core concept pages"
- **Arc** `312f0b01` "Revert Rewrite the Arc-Chronicle integration landing…" (see lesson #1 — that revert is intentional)
- **Fundamentals** `c59c939` "Rewrite Fundamentals C# landing with a concept walkthrough"
- **cli** `5b766ce` "Add task-oriented CLI scenario recipes"
- **Components** `a644746` "Add a 'choosing a component' decision guide"
...
- **More concept-page diagrams.** Confirmed still missing diagrams (project rule: a concept page without one is incomplete): `Chronicle/Documentation/concepts/{event-store,namespaces,event-type,event,constraints,projection,subject}.md`. Purely additive; build-verify after each.
...
- The OPEN backlog in §8 (tour-voice pass on Arc/Components/CLI getting-started, the Why pages, Chronicle host variants).
- **Do NOT** mass-remove the ~159 weasel-word warnings — the conversational "just/simply" usage is the deliberate tour voice; the linter is a signal, not a mandate.
...
1. **Tour-voice pass across the remaining teaching pages** — getting-started for **Arc (backend + frontend), Components, CLI** (Chronicle's is done), the **Why pages**, and the Chronicle **host variants** (`console`/`aspnetcore`/`worker`, still flat reference that `[!INCLUDE]` a big `common.md`). This is the user's **#1 qualitative bar**: walk the reader through like Marten / Wolverine / Aspire (study the local clones — §7), match the Chronicle tutorial + getting-started voice. Optional: build Aspire's `QuickStartJourney` vertical-timeline component for the Overview landings (the user liked it).

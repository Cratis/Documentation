...
Marketplace installation and the published npm package are real. The npm
package is a supported stable release; marketplace installs follow the
`Cratis/AI` default branch and do not carry a versioned support claim.
</Aside>
...
1. **Coding skills** — add `Cratis/AI` as a marketplace and install the `cratis`
   plugin ([commands per host](/ai/getting-started/)), or install the published
   [`@cratis/pi`](/ai/profiles/) package through Pi.

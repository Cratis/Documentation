...
Read a single vertical column of an event model — one screen, its command, the event, the read model behind the next screen — and you're looking at one [vertical slice](/fundamentals/): one feature folder with the command, event, projection, and React component that implement it. The [full-stack walkthrough](/build-a-full-app/) builds exactly that column, block by block.
</Aside>
...
<CardGrid>
  ...
  <SimpleCard title="How Cratis compares" icon="approve-check" link="/comparisons/" />
</CardGrid>

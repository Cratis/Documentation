...
Cursor, and `@cratis/ai-fundamentals` is published to npm as a stable release
carrying the whole public skill set. Profile subscriptions (`.cratis/ai.json`)
record a repository's exact scope and receive reviewed update pull requests as
per-profile packages pass their preview gates.

Still gated on the way to a supported release:

- the versioned per-profile package flow is not published yet;

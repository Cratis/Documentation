...
| Versioning | Follows the `Cratis/AI` default branch you already read | Pinned to exact package versions; updates arrive as reviewed pull requests *(designed — not published yet; today only `@cratis/pi` exists on npm)* |
...
4. **List your languages.** The `languages` array prevents a profile from
   loading guidance for languages the repository does not use.

5. **Keep project facts in ordinary repository documentation.** The managed
   corpus owns reusable guidance, not architecture, environment names, or
   credentials.
...
- **Pi/npm** — install the package for your profile at the exact pinned version:
  ...
  Pi records the installed version in `.pi/settings.json`; commit it after
  review.
...
your feet, and removing a package never touches `.cratis/ai`,

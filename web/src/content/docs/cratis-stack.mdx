...
description: One stack, end to end — model your domain in Studio, secure the edge with AuthProxy, build it with Arc, Chronicle, and Components, use Chronicle from .NET, TypeScript, Kotlin/Java, or Elixir, choose the right Chronicle storage, run it all from one Aspire AppHost, operate it with the CLI and Workbench, and accelerate every step with AI.
---
...
Most stacks are assembled parts you bolt together and then spend the whole project keeping in sync. Cratis is one connected ecosystem, designed together, from one company — event store, application framework, UI, tooling, and AI skills — so the domain you sketch at the top of the hour is typed C#, authenticated tenant-aware requests, a live React screen, and an inspectable event log before the hour is out. Not because you typed faster — because event sourcing is the default backbone for information systems and every handoff you'd normally hand-write is covered by conventions, generation, and open runtime boundaries. We believe that is what makes software better, more robust, and faster to deliver — for developers and for the AI agents working alongside them. [Start building →](/chronicle/get-started/) · [Why developers choose Cratis →](/why-cratis/)
</TopicHero>
...
- Or you **start from a system that already exists**, and [Prologue](/prologue/) captures what it actually does — HTTP commands, database changes, telemetry — and interprets that into the same kind of model, evidenced rather than guessed.
...
Wiring the stack into one host is itself a single call: the [`Cratis` package](/arc/backend/csharp/chronicle/cratis-package/) brings Arc and Chronicle up together with `AddCratis`/`UseCratis`, sharing storage, identity, and hosting.

How the two line up *in code* — how a command's `Handle()` return becomes an appended event, how the event source id picks the stream, and how a `[ReadModel]` projection is served back by a query — is laid out in [Integrate with Chronicle](/arc/backend/csharp/chronicle/).
...
Model in Studio, generate the slices, build them, inspect the result — an agent can take part at every step. [AI-native development](/ai/) covers the setup and exactly what an agent can do.
...
<CardGrid>
  ...
  <SimpleCard title="Prologue — capture" icon="magnifier" link="/prologue/">
    Point it at an existing system — no Cratis constructs required — and it captures HTTP commands, database changes, and telemetry, then interprets them into an event model and a Screenplay.
  </SimpleCard>
  <SimpleCard title="Chronicle — events" icon="seti:db" link="/chronicle/">
    The event-sourcing database and runtime: gRPC/protobuf boundary, .NET-first client, TypeScript, Kotlin/Java, and Elixir clients, MongoDB/PostgreSQL/SQL Server/SQLite storage, Orleans inside.
  </SimpleCard>
  ...
  <SimpleCard title="CLI — operate" icon="rocket" link="/cli/">
    Inspect and interact with a running store from the terminal: events, observers, read models, recommendations, jobs, replay, failed partitions, and diagnostics.
  </SimpleCard>
  <SimpleCard title="Workbench — inspect" icon="laptop" link="/chronicle/workbench/">
    The web-based inspection surface for Chronicle event stores — browse events, observers, projections, and read models in the browser.
  </SimpleCard>

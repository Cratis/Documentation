...
description: Cratis combines framework-aware AI workflows you can install today with documented CLI and MCP operating tools. Learn what is installable now, what stays gated, and where each agent ecosystem fits.
slug: ai
---
...
GitHub Copilot, or Cursor and install the `cratis` plugin, or pin the published
`@cratis/ai-fundamentals` npm package through Pi. Everything ships as an
unsupported `0.x` evaluation distribution — the supported tier waits on release
gates, not on packaging. Cratis-owned repositories additionally carry
repository-local rules and host adapters, so maintainers can use these workflows
in place.
...
Because the skills encode the conventions, an agent that uses them produces slices that look like the rest of your codebase — not a layered approximation of it. (The canonical source is the `Cratis/AI` repository; a repository that needs a narrower scope subscribes to [profiles](/ai/profiles/) — per product, per language, pinned.)

Start with [Start using Cratis AI](/ai/getting-started/) — one command per
host installs the plugin. The [agent harness guide](/ai/harnesses/) walks
every harness type with exact commands, the [scenarios](/ai/scenarios/solo-developer/)
pages cover solo, team, and multi-harness setups, and the
[ecosystem support matrix](/ai/ecosystems/) shows what is verified and what
remains gated. [Profiles](/ai/profiles/) explains how to select exactly the
guidance a repository needs, while the [code-analysis gates](/code-analysis/)
enforce framework contracts during the build.

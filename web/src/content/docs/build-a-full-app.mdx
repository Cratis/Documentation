...
```mermaid
flowchart LR
    Form["RegisterAuthor dialog<br/>(Components)"] -->|generated proxy| Cmd["RegisterAuthor.Handle()<br/>(Arc)"]
    Cmd -->|appends| Ev["AuthorRegistered<br/>(Chronicle)"]
    Ev -->|projection| RM["Author read model"]
    RM -->|AllAuthors query| Table["Authors table<br/>(Components)"]
```
...
- The reasoning: [Why Cratis](/why-cratis/) and [Why Arc](/arc/why-arc/).

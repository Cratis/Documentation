...
## Which languages can talk to Chronicle?

Chronicle is an event-sourcing database with a language-agnostic gRPC/protobuf boundary. The first-class client is the .NET SDK (`Cratis.Chronicle` on NuGet), and Chronicle also ships TypeScript (`@cratis/chronicle` on npm), Kotlin/Java (`io.cratis:chronicle` on Maven Central, including a Spring Boot starter), and Elixir (`cratis_chronicle` on Hex) clients. A Python client is coming soon — it is pre-release and not yet published. Any language can implement the gRPC contracts directly.

See [Chronicle in your language](/chronicle-clients/) and the [client SDK docs](/chronicle/clients/).
...
See [Chronicle storage](/chronicle/hosting/configuration/storage/), [Arc MongoDB](/arc/backend/csharp/mongodb/), and [Arc Entity Framework Core](/arc/backend/csharp/entity-framework/).
...
Start with [MediatR, MVC, and Arc](/arc/backend/csharp/coming-from-mediatr-and-mvc/) and [CRUD, EF Core, and Chronicle](/chronicle/coming-from-crud/).

## What does Cratis cost, and what license is it under?

Everything Cratis publishes today is MIT licensed and free to use — Chronicle, Arc, Components, the CLI, the clients, and the supporting libraries. There is no paid tier for the software itself. If you want dedicated, private help, see [Work with us](/work-with-us/).

## Where do I ask for help?

Use [Community and help](/community/) for open questions, [Feedback and suggestions](/feedback/) for product and docs feedback, GitHub issues for durable bugs and feature requests, and [Work with us](/work-with-us/) when you need private, dedicated time.

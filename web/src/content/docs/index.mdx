...
hero:
  ...
  tagline: "We treat event sourcing as the default architecture for information systems: capture what happened as immutable facts, project exactly the read models you need, and ship a full-stack, type-safe app — identity, tenancy, backend, React, storage choice, and operations — without the boilerplate. Open source and MIT licensed, with Chronicle clients for .NET, TypeScript, Kotlin/Java, and Elixir."
  image:
...
{/* Rotates the hero above through the platform's stories (Event Sourcing, CQRS,
    Application Framework, the Stack, Any Language, AI-Native, Studio, Screenplay, Prologue). The
    frontmatter hero is the first pane and the no-JS fallback — keep the two in sync. */}
<RotatingHero />
...
<CardGrid>
  <LinkCard
    title="Explore runnable samples"
    description="Choose a focused Chronicle, Arc + React, multi-tenant, cross-store, operations, model-first, or full Library journey."
    href="/samples/"
  />
  <LinkCard
    title="Open the sample source"
    description="Browse the projects, diagrams, run commands, and ideas to try in the Cratis Samples repository."
...
<CardGrid>
  ...
  <SimpleCard title="Open event backbone" icon="puzzle" link="/chronicle-clients/">
    Chronicle exposes a gRPC/protobuf boundary, has a first-class .NET SDK, and also ships TypeScript, Kotlin/Java, and Elixir clients — with a Python client coming soon.
  </SimpleCard>
  ...
  <SimpleCard title="Predictable for AI" icon="rocket" link="/ai/">
...
<CardGrid>
  <SimpleCard title="Chronicle" icon="seti:db" link="/chronicle/">
    The event-sourcing database and runtime — gRPC/protobuf boundary, clients for .NET, TypeScript, Kotlin/Java, and Elixir, storage choice, Orleans kernel, and rich tooling over a durable event log.
  </SimpleCard>
  ...
  <SimpleCard title="AI tooling" icon="approve-check" link="/ai/">
    Skills, rules, analyzers, CLI catalogs, and MCP support that let agents build and operate with the platform's conventions.
  </SimpleCard>
  <SimpleCard title="Prologue" icon="magnifier" link="/prologue/">
    Bring an existing system in — capture its real HTTP commands, database changes, and telemetry, and interpret them into an event model.
  </SimpleCard>
  <SimpleCard title="Studio" icon="open-book" link="/studio/">
    The collaborative modeling canvas — map commands, events, and read models with your team and generate type-safe C# from the model. Coming soon.
  </SimpleCard>
  <SimpleCard title="Specifications" icon="approve-check" link="/specifications/">
    BDD-style testing for .NET — given/when/then specs that read like the event model, with Arc and Chronicle scenario helpers.
  </SimpleCard>
  <SimpleCard title="Fundamentals" icon="seti:folder" link="/fundamentals/">
    The shared building blocks — concepts, serialization, dependency injection, and type discovery for .NET and TypeScript.
  </SimpleCard>
</CardGrid>
...
| To use Chronicle from Kotlin/Java, TypeScript, or Elixir | [Chronicle in your language](/chronicle-clients/) |
...
| To bring an existing system into Cratis, not build one from scratch | [Prologue](/prologue/) |
...
| AI guidance for building and operating the stack | [AI-native development](/ai/) |
...
| Help modeling your domain, adopting Cratis, or training your team | [Work with us](/work-with-us/) |
...
Most teams get productive from the documentation, the samples, and the community. Some want the model right the first time — because the early slices set the pattern that everything after them copies, and consistency boundaries are cheap to change now and expensive to change later.

We build Chronicle, Arc, and Components, and we work with teams directly.

<CardGrid>
  <SimpleCard title="Work with us" icon="rocket" link="/work-with-us/">
    Event modeling, Cratis adoption, architecture review, implementation help, AI-ready foundations, and training — from the people who build the stack.
  </SimpleCard>
  <SimpleCard title="Ask the community" icon="open-book" link="/community/">
    Free, open, and often faster: bring the question to Discord, where Prompter answers from these docs and the maintainers pick up what it cannot.
  </SimpleCard>
</CardGrid>

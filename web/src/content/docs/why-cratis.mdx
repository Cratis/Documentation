...
<CardGrid>
  ...
  <SimpleCard title="History you can trust" icon="seti:db" link="/chronicle/why-event-sourcing/">
    When history matters, Chronicle records immutable facts and derives read models from them, so audit and replay are part of the design.
  </SimpleCard>
...
|---|---|
| Domain behavior | Studio models it, Arc turns it into commands and queries, Chronicle records the facts when history matters |
...
| Choosing event sourcing before every feature | Arc over MongoDB/EF Core by default, Chronicle when history is worth it |
...
| A product-grade SaaS shape with edge, identity, tenant isolation, typed UI, history, and operations | **AuthProxy + Arc + Components + Chronicle + CLI** | When history is worth it |

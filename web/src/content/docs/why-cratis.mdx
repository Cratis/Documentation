...
description: "The practical reasons teams choose Cratis: one stack that fits together, strong conventions, open Chronicle boundaries, a database-agnostic event store (MongoDB, PostgreSQL, SQL Server, SQLite), full-stack contracts, end-to-end foundations, and codebases that stay predictable for people and AI."
---
...
In the age of AI, software gets built differently — and the systems that stay coherent are the ones where the pieces were designed together. Cratis is one connected ecosystem from one company: Chronicle records the facts, Arc shapes the application, Components renders it, AuthProxy secures the edge, the CLI and Workbench let you see it run, and AI skills teach assistants the same conventions developers use. Write the behavior once, keep the contract typed, run on the database you already operate — MongoDB (default), PostgreSQL, SQL Server, or SQLite — and use it from .NET, TypeScript, Kotlin/Java, or Elixir. We believe software built this way is better, more robust, faster to deliver, and more cost-effective to own. [Get started →](/chronicle/get-started/) · [Build a full-stack feature →](/build-a-full-app/)
</TopicHero>
...
<CardGrid>
  ...
  <SimpleCard title="AI can work with it" icon="rocket" link="/ai/">
...
| Chronicle client boundary | Chronicle exposes gRPC/protobuf contracts, with .NET as the first-class client and TypeScript, Kotlin/Java, and Elixir clients for other runtimes |
...
<CardGrid>
  <SimpleCard title="Language-neutral boundary" icon="puzzle" link="/chronicle/architecture/">
    Chronicle speaks gRPC/protobuf at the kernel boundary. The .NET client is the first-class experience, and Chronicle also ships TypeScript, Kotlin/Java, and Elixir clients — the JVM client includes a Spring Boot starter.
  </SimpleCard>
  <SimpleCard title="A database-agnostic event store" icon="seti:db" link="/chronicle/hosting/configuration/storage/">
    The event store is database-agnostic, with pluggable storage-provider implementations including MongoDB (default), PostgreSQL, SQL Server, and SQLite — pick the one you already operate.
  </SimpleCard>
...
| Locking event history to one application language or database engine | Chronicle's gRPC boundary, .NET/TypeScript/Kotlin/Java/Elixir clients, and MongoDB/PostgreSQL/SQL Server/SQLite storage implementations |
...
- **Chronicle on its own** is an event-sourcing database you can use from *any* .NET host — a worker, a console app, a different web framework — and from other runtimes too. The .NET client is the first-class path, but the kernel boundary is gRPC/protobuf and Chronicle also ships [TypeScript, Kotlin/Java, and Elixir clients](/chronicle-clients/) — the JVM client with a Spring Boot starter. Append events, build projections, react to them. No Arc, no React required.
...
- **Cross-cutting concerns are first-class.** [Identity and access](/arc/understanding-identity-and-access/), [authorization](/arc/backend/csharp/core/authorization/), [tenancy](/arc/backend/csharp/tenancy/), [AuthProxy](/authproxy/), and [Chronicle namespaces](/chronicle/concepts/namespaces/) are part of the platform story.
...
- **Predictable code helps humans and AI.** The conventions are documented, packaged as [AI tooling](/ai/), and enforced by [code analysis](/code-analysis/), so a new feature looks like the rest of the system.
...
**What about a throwaway prototype?** Use Cratis anyway. It is meant to be quick and easy as well as robust and feature-rich — the conventions and scaffolding make even a prototype fast to stand up, so there is no reason not to use it for throwaway work. And if the prototype lives on, you are already on a solid foundation.

When *neither* fits — a couple of static pages or a non-.NET backend — Cratis is more than you need, and that's fine.
